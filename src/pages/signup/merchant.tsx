...
export function Merchant() {
  ...
  console.log(`${process.env.REACT_APP_URL_MERCHANT}`)
  ...
  async function handleFormOnSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await api.post(`${process.env.REACT_APP_URL_MERCHANT}`, data);
      toast.success('Vendedor cadastrado com sucesso👌');
      router.push('/login')
    } catch (err) {
      toast.error('Falha ao cadastrar vendedor 🤯');
      console.error(err);
    }
  }
      ...
      <form onSubmit={handleFormOnSubmit}>
        ...
        <div className="form-check mb-4">
          ...
          <label className="form-check-label" htmlFor="flexCheckChecked">
            Ao enviar este formulário de cadastro de vendedor, li e reconheço a <Link to="/" className="label-form-link">Política de Privacidade</Link>
          </label>
        </div>
